...
class RagService(object):
    def __init__(self):
        ...
        self._retriever = self.vector_service.get_retriever()#从向量库服务中获取检索器（as_retriever），后续链内检索和外部调用都复用该检索器；
        ...
        )#定义大模型的提示词模板，核心逻辑是参考资料、对话历史、用户问题；
    ...
    def retrieve_documents(self, query: str) -> list[Document]:#用于依据的查找和返回
        return self._retriever.invoke(query)#调用检索器的invoke方法，用户调用方法传入问题，返回匹配的文档列表。
    @staticmethod
    def format_answer_basis(docs: list[Document]) -> str:#将检索到的文档列表格式化为容易读的形式
    ...
    def __get_chain(self):#获取执行链
        retriever = self._retriever        # 复用类属性的检索器
        # 定义文档格式化函数：将检索到的 Document 列表转换成字符串
        def format_document(docs: list[Document]):
            if not docs:
                return "无相关参考资料"
            formatted_str = ""
            for doc in docs:# 拼接文档内容和元数据，方便提示词使用
                formatted_str += f"文档片段：{doc.page_content}\n文档元数据：{doc.metadata}\n\n"
            return formatted_str
        def format_for_retriever(value: dict) -> str:        # 定义检索器输入格式化函数：提取用户问题
            return value["input"]
